...
// selector: keccak("claim(uint256)")[:4] = 0x379607f5
static bytes selector_claim() {
  const char *sig = "claim(uint256)";
  bytes h = keccak256((const uint8_t*)sig, strlen(sig));
  return bytes(h.begin(), h.begin() + 4);
}

// Build signed EIP-1559 (type 2) claim(nonce) tx with value=0 (pure sweat).
static bool build_claim_tx(const Wallet &w, const TxParams &p, uint64_t nonce, bytes &raw, bytes &txhash) {
  bytes data = selector_claim();
  bytes n32 = pad32(u64_to_be(nonce));
  ...
  std::vector<bytes> f = {
    ...
    rlp_bytes(p.to),
    rlp_bytes(bytes()),                        // value = 0 (pure sweat)
    rlp_bytes(data),
    rlp_list({}),                              // accessList = []
  };
